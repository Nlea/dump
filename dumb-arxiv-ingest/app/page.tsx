...
export default function Home() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (scrapedChunks.length > 0) {
      ...
      await fetch("https://dump.goalastair.com/workflow", {
        method: "POST",
        body: JSON.stringify({
      ...
      for (const [index, chunk] of scrapedChunks.entries()) {
        ...
        try {
          ...
          const response = await fetch(
            "https://dump.goalastair.com/proompt/poem",
            requestOptions,
          );

          if (!response.ok) {
            throw new Error(
              `API Error for chunk ${index + 1}: ${response.status} ${response.statusText}`,
            );
          }
        ...
        {!textChunks && !isSubmitting && (
          <div className="max-w-2xl mx-auto text-center">
            ...
            <p className="mt-3 max-w-md mx-auto text-base text-gray-600 sm:text-lg md:mt-5 md:text-xl md:max-w-3xl mb-8">
              Paste an arXiv{" "}
              <code className="text-sm bg-gray-200 px-1 rounded">/html/</code>{" "}
              URL to get started.
            </p>
        ...
        {isSubmitting && (
          <div className="text-center py-10">
            ...
            <p className="text-sm text-gray-600">
              Harnessing the power of dumbness...
            </p>
            <svg
              ...
              <div className="flex items-center gap-4">
                {textChunks && textChunks.length > 0 && (
                  <button
                    ...
                    className={`py-1 px-4 rounded-full text-sm font-medium border transition duration-150 ease-in-out ${
                      showFunVersion
                    ...
                    disabled={
                      showFunVersion &&
                      (!funTextChunks ||
                        funTextChunks.length !== textChunks.length)
                    }
                  >
                    {showFunVersion
                      ? !funTextChunks
                        ...
                        {showFunVersion ? (
                          ...
                          funTextChunks && funTextChunks[index] !== null ? (
                            ...
                            <p className="text-gray-500 italic">
                              Loading fun version (chunk {index + 1})...
                            </p>
                          )
                        ) : (
